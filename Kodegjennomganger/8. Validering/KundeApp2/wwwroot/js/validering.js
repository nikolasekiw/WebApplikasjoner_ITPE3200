function validerFornavn(fornavn){
    const regexp = /^[a-zA-ZæøåÆØÅ\.\ \-]{2,20}$/; //lager en regex
    const ok = regexp.test(fornavn); //tester for den
    if(!ok){
        $("#feilFornavn").html("Fornavnet må bestå av 2 til 20 bokstaver"); //så legger vi er ut en feilmelding om regex-en slår inn
        return false;
    }
    else{
        $("#feilFornavn").html("");
